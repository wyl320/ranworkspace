<template>
<div ref="container" class="vue-scrollbar-custom">
  <div @scroll="barScroll" ref="innerContainer" class="scrollbar-container">
    <div ref="content" class="content-container">
      <slot></slot>
    </div>
  </div>
  <div ref="barContainer" v-if="!isTopBar" :style="[barContainerStyle,{background:isHideBar?'rgba(0,0,0,0)':barContainerStyle.background}]" @mouseover="barHover" @mouseout="timeOutHideBar" class="scroll_bar_container">
    <div v-drag.dragY="{ondrag:syncContent}" ref="bar" :style="[barStyle,{opacity:isHideBar?0:1}]" class="scroll_bar">
      <slot name="barIn"></slot>
    </div>
  </div>
  <div v-else="v-else" class="topBar_container">
    <div ref="topBar" :style="topBarStyle" class="topBar">
      <slot name="topBarIn"></slot>
    </div>
  </div>
</div>
</template>
<script>
	import vDrag from '@/js/v-drag.js'
	export default {
  name: 'scroll',
  data() {
    return {
  ...
  methods: {
    ...
    setWds(){
    	this.fonts=document.documentElement.clientWidth* (100 / 1280)
    }
  },
